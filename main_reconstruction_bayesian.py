...
def compute_score(latent, vae, ground_voxel, s_factor, saxs_data):
    latent = np.array(latent).reshape(1, -1).astype(np.float32)
...
if __name__ == "__main__":
    ...
    space = [Real(mu - 10*sigma, mu + 10*sigma) for mu, sigma in group_init_parameter]
    ...
    print('R²',-result.fun)
    #print("Best latent vector:", result.x)
    best_voxel = vae.decode(np.array(result.x).reshape(1, -1)).numpy()[0, :31, :31, :31, 0]
    ...
    create_ccp4_map_(ground_voxel, f'{args.output_folder}/ground_state.ccp4', voxel_size=3.33,
                     scaling_factor_file=os.path.join(args.output_folder, 'ground_state_scale_factor.npy'))
